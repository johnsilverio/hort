...
3. **Agents.** Offered only when their credentials exist on the host: Claude Code when `~/.claude` exists, written as `claude --dangerously-skip-permissions` with `~/.claude` read-only and its completion hook enabled. The question has no default answer: Enter alone does nothing, and it waits until you press `y` or `n`, which answers at once with no Enter after it. The same holds for every yes/no question in this dialogue. A question that says what mounting the directory exposes is [planned](../roadmap.md#a-credentials-question-that-says-what-it-exposes).
